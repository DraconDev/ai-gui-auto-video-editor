...
cargo build --release
```  
The resulting binary lives at `target/release/agave`.
...
2. Create a recipe referencing:
   - `target/release/agave`
   - `assets/agave.desktop`
   - `assets/icon.svg`
3. Run the builder to produce `agave-x86_64.AppImage`.
...
- `docs/` (user guides)
and creates `release/agave-<version>.tar.gz` plus a `.sha256` checksum.
...
1. Create a Flatpak manifest (JSON/YAML) that references `com.ai.videditor`, the Freedesktop runtime, and your repo. Example module snippet:
   ```json
   {
     ...
     "sources": [{"type": "git", "url": "https://github.com/DraconDev/agave", "branch": "master"}]
   }
   ```
2. Build and export:
   ```bash
   flatpak-builder --repo=repo build-dir manifest.json
   flatpak build-bundle repo agave.flatpak com.ai.videditor
   ```
...
2. Users can install via `yay -S agave` or other AUR helpers. Include instructions for building from source if necessary.
...
- Update support playbook to reference:
  ...
  - Log locations (`~/.config/agave`, `~/.cache/agave`)
